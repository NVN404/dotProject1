...
const sections = [
  ...
  },
  {
    ...
    description:
      "High school is where students refine their skills, explore their passions, and prepare for the future. Our rigorous academics, diverse electives, and extracurricular activities help students develop independence and leadership.",
    image: "/gal6.jpg",
...
const Academics = () => {
  ...
  const handleNavigation = (direction) => {
    if (direction === "next" && activePage < sections.length - 1) setActivePage(activePage + 1);
    else if (direction === "back") activePage > 0 ? setActivePage(activePage - 1) : handleCancel();
    scrollToTop();
  };
      ...
      {activePage === null ? (
        ...
        <div ref={activePageRef} className="active-page relative min-h-screen">
          ...
          <div className="flex justify-between px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-4 bg-gray-50">
            {['Back', 'Next'].map((label, i) => {
              const isDisabled = i === 0 ? activePage === 0 : activePage === sections.length - 1;
              return (
                <button
                  key={label}
